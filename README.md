### ¡Hola! Soy Alejandro López

En transición de **Analista de Soporte Técnico (3 años)** hacia **Ingeniería de Inteligencia Artificial**, cursando el último semestre de Ingeniería de Sistemas. Combino base sólida en soporte e infraestructura con desarrollo backend y las APIs de LLMs actuales.

- 🎓 Último semestre de Ingeniería de Sistemas
- 🛠️ 3 años de experiencia como Analista de Soporte Técnico
- 🤖 Actualmente construyendo soluciones con IA aplicada (LLMs) sobre proyectos reales
- 📍 Bogotá, Colombia

---

### 🧠 Habilidades

**Lenguajes y datos**
![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PHP](https://img.shields.io/badge/-PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)

**IA / LLMs**
![OpenAI](https://img.shields.io/badge/-OpenAI%20API-412991?style=flat-square&logo=openai&logoColor=white)
![Claude](https://img.shields.io/badge/-Claude%20API-D97757?style=flat-square&logo=anthropic&logoColor=white)

**Herramientas**
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![VSCode](https://img.shields.io/badge/-VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)

**APIs**
Diseño y consumo de APIs REST para integración de servicios y modelos de IA.

---

### 🚀 Proyecto

**[SafeWorkSurvey](#)** — Sistema web para mejorar el bienestar laboral
Plataforma para evaluar el estrés laboral en empresas del sector tecnológico, que analiza las respuestas de los colaboradores mediante IA (APIs de LLM) para generar recomendaciones personalizadas y detectar factores de riesgo psicosocial. Proyecto de investigación del semillero SIIANTEC (TEINCO), actualmente en continuación.
Stack: PHP · SQL · APIs de LLM

*(En construcción — repositorio próximamente)*

---

### 📫 Contactame
Alejandro3456ca@gmail.com
Se realizan Trabajos: 3183514004

<!-- Agrega aquí tus links: LinkedIn, correo, portafolio -->
